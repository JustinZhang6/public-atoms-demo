...
核心功能开发约 **1 小时**（19:48 ~ 20:49），含需求分析、架构搭建、全链路实现。后续约 2 小时用于 AI 后端切换、代码清理、测试验收、文档撰写与部署。

| 阶段 | 时间 | 内容 |
| --- | --- | --- |
| 需求分析 + atoms.dev 调研 | ~10min | 阅读文档站，明确核心功能范围 |
| 工程搭建 + 全链路实现 | ~50min | Codex 完成 4 个里程碑 commit（工程初始化 → 持久化 → 工作台 → 文档） |
| AI 后端切换 + 测试 | ~1h | 切换为实际可用的 AI 后端，浏览器自动化测试 8 项全通过 |
| 文档 + 部署 | ~1h | 代码清理、交付文档、cloudflared 隧道部署 |
